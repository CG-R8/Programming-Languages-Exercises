...
quadratic_roots(A, B, C, [Root1, Root2]):-
    ...
    Root1 is (-B + Discr) / (2 * A),
    Root2 is (-B - Discr) / (2 * A).
...
naive_flatten_tree(leaf(V), [V]).
naive_flatten_tree(tree(T1, V, T2), Flattened):-
    naive_flatten_tree(T1, F1),
    naive_flatten_tree(T2, F2),
    append(F1, [V], X1),
    append(X1, F2, Flattened).
...
flatten_tree(Tree, Flattened):-
    flatten_tree(Tree, [], Xs),
    reverse(Xs, Flattened).
flatten_tree(leaf(V), Acc, [V|Acc]).
flatten_tree(tree(T1, V, T2), Acc, Flattened):-
    flatten_tree(T1, Acc, X1),
    flatten_tree(T2, [V|X1], Flattened).

%Alternate implementation: avoid the use of reverse/2 by traversing the
%tree in reverse order.
alt_flatten_tree(Tree, Flattened):-
    alt_flatten_tree(Tree, [], Flattened).
alt_flatten_tree(leaf(V), Acc, [V|Acc]).
alt_flatten_tree(tree(T1, V, T2), Acc, Flattened):-
    alt_flatten_tree(T2, Acc, X2),
    alt_flatten_tree(T1, [V|X2], Flattened).
...
regex_match(Regex, String):-
    regex_match(Regex, String, []).
regex_match(Regex, [C|Cs], Cs):-
    Regex = [_|_],
    member(C, Regex).
regex_match(range(C1, C2), [C|Cs], Cs):-
    C1 =< C,
    C =< C2.
regex_match(A + _, String, StringX):-
    regex_match(A, String, StringX).
regex_match(_ + B, String, StringX):-
    regex_match(B, String, StringX).
regex_match(A * B, String, StringX):-
    regex_match(A, String, StringX1),
    regex_match(B, StringX1, StringX).
regex_match(optional(A), String, StringX):-
    regex_match(A, String, StringX).
regex_match(optional(_), String, String).
regex_match(closure0(A), String, StringX):-
    regex_match(A, String, StringX1),
    regex_match(closure0(A), StringX1, StringX).
regex_match(closure0(_), String, String).
...
%States = [s0, s2, s3, s3],
%Z = reject ;
%*Note that the above solution does not include the state when 
%*the input becomes empty. A better solution would be:
%*N = ...
%*States = [s0, s2, s3, s3, s3],
%*Z = reject ;    
%N = ,,,
...
%States = [s0],
%Z = reject.
nfa_simulate(nfa(State, _, Finals), [], [State], accept):-
    member(State, Finals).
%For consistency and a better solution as mentioned above, the below rule
%could be changed to:    
%nfa_simulate(nfa(State, _, Finals), [], [State], reject):-    
%   \+ member(State, Finals).
nfa_simulate(nfa(State, _, Finals), [], [], reject):-
    \+ member(State, Finals).
nfa_simulate(nfa(State, Transitions, Finals), [I|Inputs], [State|States], Z):-
    member(transition(State, I, StateI), Transitions),
    nfa_simulate(nfa(StateI, Transitions, Finals), Inputs, States, Z).
nfa_simulate(nfa(State, Transitions, Finals), Inputs, [State|States], Z):-
    member(transition(State, epsilon, StateI), Transitions),
    nfa_simulate(nfa(StateI, Transitions, Finals), Inputs, States, Z).
nfa_simulate(nfa(State, Transitions, _), [I|_], [State], reject):-
    \+ member(transition(State, I, _), Transitions),
    \+ member(transition(State, epsilon, _), Transitions).
